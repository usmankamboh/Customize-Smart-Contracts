...
contract Launchpad {
    ...
    public payable returns (bool){
        ...
        require(pre.tokenAddress != _tokenAddress,"already presale done");
    ...
    function investWithUSDT(uint256 _count, uint256 _amount)public returns(uint256){
        details memory pre = presaleDetails[_count];
        IERC20 _tokenAddress = IERC20 (pre.tokenAddress);
        uint256 _tokenamount = _amount.div(pre.price);
        require (_tokenAddress.balanceOf(address(this)) >= _amount,"tokens are not enough");
        require(_amount <= pre.maximumInvestment,"increase maximum investment ");
        require(_amount >= pre.minimumInvestment,"less than minimum investment ");
        transferusdt(msg.sender,pre.seller, _amount);
        _tokenAddress.transfer(msg.sender,_tokenamount);
        pre.remainingAmount = pre.amount - _tokenamount;
    ...
    function transferusdt(address from,address to,uint256 amount) internal {
        usdtAddress.transferFrom(from,to,amount);
    }
}